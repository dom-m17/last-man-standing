...
func (r *mutationResolver) Login(ctx context.Context, input graphmodels.LoginInput) (*graphmodels.LoginResponse, error) {
	_, _, _, err := r.UserService.Login(ctx, input)
	if err != nil {
		return &graphmodels.LoginResponse{}, fmt.Errorf("logging in: %w", err)
	}

	return &graphmodels.LoginResponse{}, nil
}
